...
if [ $# -lt 1 ]; then
        echo "Usage : export_addok_fr.sh <outPath> <pythonPath>"
        echo "Exemple : export_addok_fr.sh /home/ban/test /venv/bin/python3.4"
        exit 1
fi
set -x
...
do
	echo $dep
	if [ ! -f $exportPath"/ban_addok_"$dep".json" ]; then
		$pythonPath $currentDir"/export_addok.py" $exportPath $dep
	fi
done
