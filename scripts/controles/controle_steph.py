...
def insert_multiple_item(kind, resources):
    ...
    for cci in cc:
        ...
        #cur.execute("select * from controles.resource, controles.item, controles.metadata where resource.id_item=item.id and item.id_meta=metadata.id and metadata.kind like '{}';".format(kind))
        #res_insert = cur.fetchall()
        #if res[0] not in res_insert and res[2] not in res_insert:
        #    cur.execute("INSERT INTO controles.item VALUES (DEFAULT, %s, %s) RETURNING id;", (meta_id, res[4]))
        #    item_id = cur.fetchone()[0]
        #    cur.execute("INSERT INTO controles.resource VALUES (DEFAULT, %s, %s, %s, %s)", (res[0], res[1], resource_type, item_id))
        #    cur.execute("INSERT INTO controles.resource VALUES (DEFAULT, %s, %s, %s, %s)", (res[2], res[3], resource_type, item_id))
...
def check_pile():
    ...
    cur.execute("""create table position90 as select p.*, st_transform(p.center,2154) as c2154 from position p, housenumber h, "group" g, municipality m
...
#        AND NOT h1.pk=h2.pk;"""
#    )
    cur.execute("SELECT p1.pk, p1.version, p2.pk, p2.version,'' FROM position90 p1, position90 p2, housenumber h1, housenumber h2 WHERE p1.housenumber_id=h1.pk AND p2.housenumber_id=h2.pk AND p1.source_kind=p2.source_kind AND p1.c2154 && st_buffer(p2.c2154,5) AND st_distance(p1.c2154, p2.c2154)<5 AND NOT h1.pk=h2.pk;")
    resources = cur.fetchall()
