...
def db_connect():
    try:
        conn = psycopg2.connect("dbname='ban_prod_20170920' user='ban'")
        return conn
...
def init():
    ...
    try:
        ...
        cur.execute("CREATE TABLE controles.item(id serial PRIMARY KEY, id_meta integer REFERENCES controles.metadata (id));")
